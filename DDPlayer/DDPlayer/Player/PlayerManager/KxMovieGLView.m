...
- (void) resolveUniforms: (GLuint) program{
    _uniformSampler = glGetUniformLocation(program, "s_texture");//所谓location，相当于是序号。
}
...
- (void) setFrame: (KxVideoFrame *) frame{
    ...
    /*
     ---glPixelStorei用于设置像素存储模式。void glPixelStorei(GLenum pname,GLint param);
     它含有两个参数：
    ...
    glTexImage2D(GL_TEXTURE_2D,
                 ...
                 GL_RGB,
                 GL_UNSIGNED_BYTE,
                 rgbFrame.rgb.bytes);//glTexImage2D的width、height的值，老版本中必须是2的整数次方，新版中为了性能，最好是。
    
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);//图象从纹理图象空间映射到帧缓冲图象空间(映射需要重新构造纹理图像,这样就会造成应用到多边形上的图像失真),这时就可用glTexParmeteri()函数来确定如何把纹理象素映射成像素.
...
//    获取一个一致变量的存储位置只需要给出其在shader中定义的变量名即可：GLint glGetUniformLocation(GLuint program, const char *name);
